...
class DelayedSellEnv(ANMEnv):
    ...
    def __init__(self):
        ...
        # Maximum generation and energy price 24-hour time series.
        self.P_maxs = _get_gen_time_series()
        self.G_prices = _get_price_time_series()

    def init_state(self):
        n_dev, n_gen, n_des = 3, 1, 1
    ...
    def next_vars(self, s_t):
        ...
        vars = []
        # Append load time series (if applicable)

        # Append gen time series (if applicable)
        for p_max in self.P_maxs:
            vars.append(p_max[aux])

        # Append grid price time series (if applicable)
        for g_price in self.G_prices:
            vars.append(g_price[aux])
        # Redundant, only ever 1 grid price, can do this with a simpler method if needed, only copied for consistency.

        # Append auxiliary variable(s) (if applicable)
        vars.append(aux)
...
def _get_gen_time_series():
    ...
    # Device 1 (wind farm).
    s1 = 50 * np.ones(25)
    s12 = np.linspace(46.375, 4.625, 7)
    s2 = 0 * np.ones(13)
